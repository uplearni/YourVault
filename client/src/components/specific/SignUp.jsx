    ...
    useEffect(() => {
      ...
      const handleSignup=async (e)=>{
        ...
        try{
          ...
          const loginResponse = await api.post('/user/login', { email: formData.email, password: formData.password });
          const { token, email } = loginResponse.data;
          authStore.getState().setAuth(token, userId, name, email);
        ...
        <div className="flex flex-col justify-evenly gap-y-6 px-4 w-full">
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-light-text dark:text-dark-text">
              Enter Password ( Minimum Length - 6 )
            </label>
