  ...
  exports.createCollection=async (req,res,next)=>{
    ...
    if (cname) {
      cname = cname.toLowerCase().split(' ').filter(Boolean).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    }
    if (description) {
      description = description.toLowerCase().split(' ').filter(Boolean).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    }
    try{
        if(!cname){
